...
WITH RECURSIVE 
-- Generate date series from March 21, 2023 to current date + 7 days
date_range AS (
  SELECT CAST('2023-03-21' AS DATE) as date
  UNION ALL
...
base_calculations AS (
  SELECT 
    ...
    END + DAY(date) AS doy_g,
    -- Leap year calculation parameters
    CAST(MOD(YEAR(date), 4) AS SIGNED) AS d_4,
    FLOOR(MOD((YEAR(date) - 16), 132) * 0.0305) AS d_33
  FROM date_range
),
...
final_date AS (
  SELECT
    ...
    CASE
      ...
      ELSE doy_g_adj + a
    END AS doy_j,
    -- Calculate the day of week (MySQL: 1 = Sunday, 7 = Saturday)
    MOD(DAYOFWEEK(DATE) + 5, 7) + 1 AS dow
...
persian_dates AS (
  SELECT
    ...
    CASE
      WHEN doy_j < 187 THEN FLOOR((doy_j - 1) / 31) + 1  -- First 6 months (31 days each)
      ELSE FLOOR((doy_j - 187) / 30) + 7                 -- Last 6 months (30 days each)
    END AS persian_month,
    -- Calculate Persian day of month
    CASE
      WHEN doy_j < 187 THEN doy_j - (FLOOR((doy_j - 1) / 31) * 31)
    ...
    CASE DAYOFWEEK(DATE)
      WHEN 7 THEN DATE -- If it's already Saturday, use current date
      ELSE DATE_SUB(DATE, INTERVAL(DAYOFWEEK(DATE)) DAY) -- Go back to previous Saturday
    END AS persian_week_start_date
  FROM final_date
)

-- Final selection with Persian calendar components
SELECT
  date,
  day_start,
  persian_year,
