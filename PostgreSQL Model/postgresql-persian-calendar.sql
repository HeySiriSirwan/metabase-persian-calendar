...
- Adds week number of year (persian_week_of_year)
- **New:** Adds weekday name in Persian (weekday)
*/

WITH RECURSIVE persian_calendar AS (
  -- Generate date series from 2022-03-21 until 2026-03-20 (end of Persian year 1404)
  WITH date_range AS (
  ...
  SELECT 
    ...
    -- Calculate the week number of the Persian year.
    FLOOR((persian_week_start_date - persian_new_year_week_start) / 7) + 1 AS persian_week_of_year,
    -- New: Add weekday name in Persian based on the calculated dow
    CASE 
      WHEN dow = 6 THEN 'شنبه'
      WHEN dow = 0 THEN 'یکشنبه'
      WHEN dow = 1 THEN 'دوشنبه'
      WHEN dow = 2 THEN 'سه‌شنبه'
      WHEN dow = 3 THEN 'چهارشنبه'
      WHEN dow = 4 THEN 'پنجشنبه'
      WHEN dow = 5 THEN 'جمعه'
    END AS persian_weekday,
    CASE 
      WHEN dow = 6 THEN 1
      WHEN dow = 0 THEN 2
      WHEN dow = 1 THEN 3
